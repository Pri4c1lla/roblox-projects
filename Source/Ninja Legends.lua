...
run(function()if getgenv().Shion_Loaded then return end end) -- If you're developer then, Remove this line.
...
run(function() pcall(function() getgenv().Shion_Loaded = true end) end)
...
env.SettingUi = {}; env.SettingUi = {
    ...
    ["Acrylic"] = true,
    ["Transparency"] = true,
    ["RenderingMode"] = "RenderStepped", -- "Heartbeat", "RenderStepped", "Stepped"
...
local Script = {
    ...
    Boss = {
        ["SelectBoss"] = {},
        ["KillBoss"] = false, -- i forgot to add 😴
        ["InstantBoss"] = false,
...
run(function()
    local suc , fail = pcall(function()
        ...
        for i,v in next, workspace:WaitForChild("islandUnlockParts"):GetChildren() do
...
local function CheckSkill(skill,group)
    if group == "Dark" then
        for i,v in pairs(game:GetService("Players").LocalPlayer.ownedDarkSkills:GetChildren()) do
            if v:FindFirstChild(skill) then
                return true
            end
        end
    elseif group == "Light" then
        for i,v in pairs(game:GetService("Players").LocalPlayer.ownedLightSkills:GetChildren()) do
            if v:FindFirstChild(skill) then
                return true
            end
        end
...
local function GetChestName()
    for i,v in pairs(workspace:GetChildren()) do
        if Players.LocalPlayer:IsInGroup(4705120) then
            if v:IsA("Model") and string.match(v.Name,"Chest") and not string.match(v.Name,"evil") and not string.match(v.Name,"light") and v.circleSignPart.signGui.timeLabel.text == "Ready To Collect" then
                return v
            end 
        else
...
local function randomteleport(Part)
    if RandomType == 1 then
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = Part.HumanoidRootPart.CFrame * CFrame.new(0, 15, 1)
    ...
    elseif RandomType == 3 then
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = Part.HumanoidRootPart.CFrame * CFrame.new(1, 1, -15)
    elseif RandomType == 4 then
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = Part.HumanoidRootPart.CFrame * CFrame.new(15, 1, 0)
...
run(function()
    ...
    LoopWalkSpeed:OnChanged(function(Value)
        ...
        while Script.Players.WalkSpeed do task.wait()
    ...
    local RenderRadius = Tabs.Misc:AddSlider("Range", {
        ...
        Rounding = 50,
        Callback = function(Value)
            Script.Misc.RangeRadius = tonumber(Value)
        end
    })
    ...
    local PerformanceSection = Settings:AddSection("Performance")
    ...
    PerformanceSection:AddDropdown("RenderingMode", {
        ...
        Default = env.SettingUi.RenderingMode,
        Callback = function(Value)
            env.SettingUi.RenderingMode = Value
            Window:Dialog({
                Title = "Warn",
...
run(function()
    RunService[env.SettingUi.RenderingMode]:Connect(function()
                ...
                for _, v in pairs(workspace.shurikensFolder:GetChildren()) do
                    if v.Name == "Handle" then
                        if v:FindFirstChild("BodyVelocity") then
                            ...
                            bv.Velocity = getMouse.Hit.lookVector * math.random(1000)
                        end
                    end
                end
                ...
                for _, v in pairs(workspace.shurikensFolder:GetChildren()) do
                    if v.Name == "Handle" then
                        if v:FindFirstChild("BodyVelocity") then
                            local bv = v:FindFirstChildOfClass("BodyVelocity")
                            bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                            bv.Velocity = env.PositionTarget
                        end
                    end
                end
            end
            ...
            if Script.Misc.KillAura then
                for _,v in next, Players:GetPlayers() do
                    if v.Name ~= lp.Name then
                        ...
                        if lp:DistanceFromCharacter(v.Character.HumanoidRootPart.Position) <= Script.Misc.RangeRadius and not v.Character:FindFirstChild("inSafezone") and not getchar():FindFirstChild("inSafezone") then
        ...
        pcall(function()
            ...
            then
                if HumanoidRootPart() then
                    ...
                    if LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid").Sit == true then
                        LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid").Sit = false
                    end

                    if not HumanoidRootPart():FindFirstChild(env._trash_table.Characters.Velocity) then
                        local Velocity = Instance.new("BodyVelocity",HumanoidRootPart()) -- Set Parent to Humanoidrootpart
                        Velocity.Name = env._trash_table.Characters.Velocity
                        Velocity.MaxForce = Vector3.new(math.huge,math.huge,math.huge);
                        Velocity.Velocity = Vector3.new(0,0,0);
                    elseif HumanoidRootPart():FindFirstChild(env._trash_table.Characters.Velocity) then
                        ...
                        --// Another Stuff
                        for i,y in ipairs(lp.Character:GetChildren()) do
                ...
                if lp.Character:FindFirstChild("HumanoidRootPart"):FindFirstChild(env._trash_table.Characters.Velocity) then
                    lp.Character:FindFirstChild("HumanoidRootPart"):FindFirstChild(env._trash_table.Characters.Velocity):Destroy();
                end
                for i,y in ipairs(lp.Character:GetChildren()) do
                    if y:IsA("Humanoid") then
                        y.MaxSlopeAngle = 89
                        y.WalkSpeed = env.oldwalkspeed
                    end
                end
            end
